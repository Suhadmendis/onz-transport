...
if ($_GET["Command"] == "pass_quot") {
    ...
    $sql = "Select * from payinfo where pay_ref ='" . $cuscode . "'";
    ...
    if ($row = $sql->fetch()) {
        ...
        $ResponseXML .= "<id><![CDATA[" . $row['pay_ref'] . "]]></id>";
        $ResponseXML .= "<str_customername1><![CDATA[" . $row['pdate'] . "]]></str_customername1>";
...
if ($_GET["Command"] == "search_custom") {
    ...
    $sql = "Select * from payinfo where pay_ref<> ''";

    if ($_GET['cusno'] != "") {
        $sql .= " and pay_ref like '%" . $_GET['cusno'] . "%'";
    }
    ...
    foreach ($conn->query($sql) as $row) {
        ...
        $ResponseXML .= "<tr> 
                              <td onclick=\"custno('$cuscode');\">" . $row['pay_ref'] . "</a></td>
